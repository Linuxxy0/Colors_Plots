        ...
        <div className="rounded-[28px] border p-5" style={{ backgroundColor: theme.panel, borderColor: `${theme.muted}22` }}>
          <div className="flex items-center justify-between">
            <div>
              ...
              <div className="mt-1 text-sm" style={{ color: theme.muted }}>{language === 'zh' ? '这里展示主题在真实科研界面中的层级关系。' : 'See how the palette behaves in a real research UI.'}</div>
            </div>
            <div className="rounded-full px-3 py-1 text-xs font-medium" style={{ backgroundColor: `${theme.accent}12`, color: theme.accent }}>{language === 'zh' ? '配色预览' : 'Palette preview'}</div>
          </div>
            ...
            <div className="rounded-[24px] border p-5" style={{ borderColor: `${theme.muted}18`, backgroundColor: theme.panel }}>
              <div className="grid gap-4 md:grid-cols-2">
                <div className="rounded-[22px] p-4" style={{ backgroundColor: theme.background }}>
                  ...
                  <div className="mt-3 text-sm leading-6" style={{ color: theme.muted }}>{language === 'zh' ? '不是动态图表，而是主题在真实面板里的视觉层次。' : 'Not chart animation, but a real UI hierarchy preview for the theme.'}</div>
                </div>
                <div className="rounded-[22px] border p-4" style={{ borderColor: `${theme.muted}18` }}>
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    {theme.tags.map((tag) => (
                      <span key={tag.en} className="rounded-full px-3 py-1 text-xs font-medium" style={{ backgroundColor: `${theme.accent}10`, color: theme.accent }}>{t(tag, language)}</span>
                    ))}
                  </div>
                </div>
              </div>
        ...
        <div className="space-y-4">
          <div className="rounded-[28px] border p-5" style={{ backgroundColor: theme.panel, borderColor: `${theme.muted}22` }}>
            ...
            <div className="mt-4 space-y-3">
              {[
                ...
                [language === 'zh' ? '主文字' : 'Text', theme.foreground],
          ...
          <div className="rounded-[28px] border p-5" style={{ backgroundColor: theme.panel, borderColor: `${theme.muted}22` }}>
            ...
            <ul className="mt-4 space-y-3 text-sm leading-7" style={{ color: theme.foreground }}>
              <li>{language === 'zh' ? '适合论文插图时保留低饱和主色与高留白。' : 'Keep low-saturation primaries and whitespace for paper figures.'}</li>
              <li>{language === 'zh' ? '适合投屏时提高文字与背景对比。' : 'Increase text/background contrast for projected screens.'}</li>
              <li>{language === 'zh' ? '导出时优先 SVG 或高分辨率 PNG。' : 'Prefer SVG or high-resolution PNG when exporting.'}</li>
            </ul>
          </div>
        </div>
      </div>

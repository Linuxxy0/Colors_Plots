...
export function RadarPreview({ records, theme, mode = 'card' }: ChartPreviewProps) {
  ...
  const [info, setInfo] = useState<HoverInfo>({ primaryLabel: 'metric', primaryValue: keys[0] ?? '—', secondaryLabel: 'score', secondaryValue: formatMetric(values[0] ?? 0) });
